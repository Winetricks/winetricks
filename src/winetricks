...
# - 'checkbashism -p -x winetricks' should show no warnings (per Debian policy)
...
w_steam_safemode()
{
    cat > "$W_CACHE/steamconfig.pl" <<"_EOF_"
#!/usr/bin/perl
...
do
    cp "$file" "$file.old"
    perl "$W_CACHE"/steamconfig.pl "$file.old" > "$file"
done
}
...
w_steam_install_game()
{
    ...
    w_ahk_do "
        ...
        winwait Steam  ; wait for login window
        winwaitclose
...
load_adobeair()
{
    ...
    w_download http://airdownload.adobe.com/air/win/download/2.6/AdobeAIRInstaller.exe 3d9c2f9d8f3533424cfea84d61fcb9464278d9fc
    cd "$W_CACHE"/adobeair
...
load_dotnet20()
{
    ...
    then
        w_try $WINE dotnetfx.exe
    else
        w_try $WINE dotnetfx.exe /q /c:"install.exe /q"
    fi
    w_unset_winver
...
load_dotnet20sp2()
{
    ...
    w_try $WINE "$W_CACHE"/dotnet20sp2/NetFx20SP2_x86.exe 
    # FIXME: add ${W_OPT_UNATTENDED:+ /q /c:"install.exe /q"}
    # once we fix the exit status 67, or if that turns out to be harmless.
...
load_dotnet35()
{
    ...
    *) w_warn "dotnet35 does not yet fully work or install on wine.  Caveat emptor." ;;
    ...
    w_download http://download.microsoft.com/download/6/0/f/60fc5854-3cb8-4892-b6db-bd4f42510f28/dotnetfx35.exe
...
w_metadata flash dlls \
    ...
    installed_file1="$W_SYSTEM32_DLLS_WIN/Macromed/Flash/FlashUtil10u_Plugin.exe" \
    installed_file2="$W_SYSTEM32_DLLS_WIN/Macromed/Flash/FlashUtil10u_ActiveX.exe" \
...
load_flash()
{
    ...
    w_download http://fpdownload.macromedia.com/get/flashplayer/current/install_flash_player_ax.exe da3f4b28f77a44b25cfb1453278683b888904714
    cd "$W_CACHE"/flash
    ...
    w_download http://fpdownload.macromedia.com/get/flashplayer/current/install_flash_player.exe 8c14f60fa625d26698afceb1e234eaae4637389e
...
w_metadata kindle apps \
    ...
    installed_exe1="$W_PROGRAMS_WIN/Amazon/Kindle For PC/Kindle.exe" \
    homepage="http://www.amazon.com/gp/feature.html/?docId=1000426311"

load_kindle()
{
    w_download http://kindleforpc.amazon.com/32337/KindleForPC-installer.exe \
        2ad916877f418ceb8bdc4ae13b0c5e9099ffb8d7
    cd "$W_CACHE"/kindle
    w_try $WINE $file1 ${W_OPT_UNATTENDED:+ /S}
    w_declare_exe "$W_PROGRAMS_WIN\\Amazon\\Kindle For PC" Kindle.exe
}
...
w_metadata mono210 dlls \
    ...
    year="2011" \
    media="download" \
    file1="mono-2.10.2-gtksharp-2.12.10-win32-5.exe" \
    installed_file1="$W_PROGRAMS_X86_WIN/Mono-2.10.2/bin/mono-2.0.dll"

load_mono210()
{
    w_download http://ftp.novell.com/pub/mono/archive/2.10.2/windows-installer/5/mono-2.10.2-gtksharp-2.12.10-win32-5.exe 630769a5bf70d025308c994920d3826dbadb03f5
    cd "$W_CACHE"/mono210
...
load_shockwave() {
    ...
    w_download http://fpdownload.macromedia.com/get/shockwave/default/english/win95nt/latest/sw_lic_full_installer.msi 7fd6cc61bb20d0bef654a44f4501a5a65b55b0c9
...
w_metadata borders apps \
    title="Borders Desktop (Kobo e-book reader)" \
    publisher="Kobo" \
    year="2011" \
    media="download" \
    file1="KoboSetup.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/Borders Desktop/Borders Desktop.exe" \
    homepage="http://www.borders.com/online/store/MediaView_ereaderapps"

load_borders()
{
    w_download http://download.kobobooks.com/desktop/1/KoboSetup.exe
    cd "$W_CACHE"/borders
    w_try $WINE $file1 ${W_OPT_UNATTENDED:+ /S}
    w_declare_exe "$W_PROGRAMS_X86_WIN\\Borders Desktop" "Borders Desktop.exe"
}
...
w_metadata firefox36 apps \
    title="Firefox 3.6 (broken in wine)" \
    publisher="Mozilla" \
    year="2011" \
    media="download" \
    file1="Firefox Setup 3.6.18.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/Mozilla Firefox/firefox.exe"

load_firefox36()
{
    w_download "http://download.mozilla.org/?product=firefox-3.6.18&os=win&lang=en-US" c6673047eec8b909fad67f387e3ef8f8f43d54e3 "$file1"
...
w_metadata firefox apps \
    title="Firefox 5 (broken in wine)" \
    publisher="Mozilla" \
    year="2011" \
    media="download" \
...
load_nook()
{
    ...
    w_download \
        ...
        4a06a529b93ed33c3518326d874b40d8d7b70e7a
    cd "$W_CACHE"/nook
...
w_metadata opera apps \
    ...
    year="2011" \
    media="download" \
    file1="Opera_1111_en_Setup.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/Opera/opera.exe"

load_opera()
{
    w_download ftp://ftp.opera.com/pub/opera/win/1111/en/Opera_1111_en_Setup.exe 36c34feb0fa7e4611bb0480a38d51a16bc9443d8
    cd "$W_CACHE"/$W_PACKAGE
...
w_metadata spotify apps \
    ...
    installed_exe1="$W_PROGRAMS_X86_WIN/Spotify/spotify.exe"
...
load_spotify()
{
    ...
    w_ahk_do "
        ...
        WinWait, Spotify, Location
        if ( w_opt_unattended > 0 ) {
            Sleep 500
            ControlClick Button7 ; Start after install
            Sleep 500
            ControlClick Button2 ; Install
            WinWait, Spotify, Complete
            Sleep 500
    ...
    w_declare_exe "$W_PROGRAMS_X86_WIN\\Spotify" "spotify.exe"
}
...
w_metadata songbird apps \
    ...
    year="2010" \
    media="download" \
    file1="Songbird_1.9.3-1959_windows-i686-msvc8.exe" \
...
{
    w_download \
        http://download.songbirdnest.com/installer/windows/i686-msvc8/Songbird_1.9.3-1959_windows-i686-msvc8.exe \
        1e6bdc9d5681f44320987cc73cde66f85b3e8032
    cd "$W_CACHE/songbird"
    w_try $WINE $file1 ${W_OPT_UNATTENDED:+ /S}
...
w_metadata winamp apps \
    ...
    year="2011" \
    media="download" \
    file1="winamp561_full_emusic-7plus_en-us.exe" \
...
load_winamp()
{
    ...
    w_download \
        http://download.nullsoft.com/winamp/client/winamp561_full_emusic-7plus_en-us.exe 21eeab7c68beba0acd59ec452b772641ae784d0a
    cd "$W_CACHE/$W_PACKAGE"
    if test $W_OPT_UNATTENDED
    then
        w_ahk_do "
...
load_bladekitten_demo()
{
    ...
    then
        w_download "http://downloads.megagames.com/download.php?file=BladeKittenDemoInstall.exe" 8c41706ff3ad029317e5eb6853c34732498d97c7 BladeKittenDemoInstall.exe
    fi
...
w_metadata digitanks_demo games \
    ...
    media="download" \
    file1="digitanks.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/Digitanks/digitanks.exe" \
    homepage="http://www.digitanks.com"

load_digitanks_demo()
{
    # 8 june 2011: f204b13dc64c1a54fb1aaf27187c6083ebb16acf
    w_download "http://static.digitanks.com/files/digitanks.exe" \
        f204b13dc64c1a54fb1aaf27187c6083ebb16acf
    cd "$W_CACHE/$W_PACKAGE"
    w_try "$WINE" $file1 ${W_OPT_UNATTENDED:+ /S}
    ...
    w_declare_exe "$W_PROGRAMS_X86_WIN\\Digitanks" digitanks.exe
}
...
load_dragonage2_demo()
{
    ...
    w_ahk_do "
        ...
        winwait, Dragon Age II Demo Setup, Completed
        if ( w_opt_unattended > 0 ) {
            send {Enter}
...
w_metadata hon games \
    ...
    year="2010" \
    media="download" \
    file1="HoNClient-2.0.36v2.exe" \
    installed_exe1="$W_PROGRAMS_X86_WIN/Heroes of Newerth/hon.exe"

load_hon()
{
    w_download http://dl.heroesofnewerth.com/HoNClient-2.0.36v2.exe \
               0e1889279dc07179eeb59e29f803b64d75b688c4
...
load_hegemonygold_demo()
{
    ...
    w_download "http://www.longbowgames.com/downloads/Hegemony%20Gold%20Installer.exe" 4069656ea3c3760b67d1c5adff37de7472955f72 HegemonyGoldInstaller.exe
...
w_metadata imvu games \
    ...
    year="2004" \
    media="download" \
    file1="InstallIMVU_460.0_st_c.exe" \
    installed_exe1="c:/users/$LOGNAME/Application Data/IMVUClient/IMVUClient.exe"

load_imvu()
{
    w_download http://static-akm.imvu.com/imvufiles/installers/InstallIMVU_460.0_st_c.exe
...
w_metadata mb_warband_demo games \
    ...
    year="2010" \
    media="download" \
    file1="mb_warband_setup_1134.exe" \
...
load_mb_warband_demo()
{
    ...
    w_download "http://download.taleworlds.com/mb_warband_setup_1134.exe" 1aac92b6c97869350b80fe7a08f559719deb37d7

    cd "$W_CACHE/$W_PACKAGE"
    w_ahk_do "
        SetTitleMatchMode 2
        run mb_warband_setup_1134.exe
        winwait Warband
        if ( w_opt_unattended > 0 ) {
            controlclick button2
            winwait Warband
...
load_nfsworld()
{
    ...
    w_download http://static.cdn.ea.com/blackbox/u/f/NFSWO/Launcher/weblaunch_1.8.35.441/setup_441.exe 0f75ea12fc3fe40dd97bf0b6556ed146de435edf
...
load_tropico3_demo()
{
    ...
    if w_workaround_wine_bug 24845 "disabling mmdevapi to fix sound"
    then
...
load_wglgears()
{
    w_download http://www.cs.nmsu.edu/~jeffery/win32/wglgears.exe d65d2098bc11af76cb614946342913b1af62924d
