...
# $4 - [dxgi,][d3d9,][d3d10,]d3d11 [d3d9=dvk9 d3d10/d3d11=dxvk] (required)
helper_dxvk_d9vk()
{
    ...
    case "${_W_dll_overrides}" in
        *d3d9*)
            # dvk9 repository, for d3d9 support
            _W_repository="Joshua-Ashton/d9vk"
            if [ "${_W_package_version}" != "master" ]; then
            ...
            ;;
        *)
            # dxvk repository, for d3d10/d3d11 support
    ...
    if ! w_wine_version_in ",${_W_min_wine_version}" ; then
        w_warn "${_W_repository#*/} ${_W_package_version} does not support wine version ${_wine_version_stripped}. ${_W_repository#*/} ${_W_package_version} requires wine version ${_W_min_wine_version} (or newer). Vulkan ${_W_min_vulkan_version} API (or newer) support is recommended."
    fi
...
load_dxvk070()
{
    ...
    helper_dxvk_d9vk "$file1" "3.18" "1.1.80" "dxgi,d3d10,d3d11"
}
...
load_dxvk072()
{
    ...
    helper_dxvk_d9vk "$file1" "3.18" "1.1.84" "dxgi,d3d10,d3d11"
}
...
load_dxvk090()
{
    ...
    helper_dxvk_d9vk "$file1" "3.19" "1.1.87" "dxgi,d3d10,d3d11"
}
...
load_dxvk093()
{
    ...
    helper_dxvk_d9vk "$file1" "4.0" "1.1.93" "dxgi,d3d10,d3d11"
}
...
load_dxvk100()
{
    ...
    helper_dxvk_d9vk "$file1" "4.5" "1.1.101" "dxgi,d3d10,d3d11"
}
...
load_dotnet20()
{
    ...
    if [ "$W_ARCH" = "win32" ]; then
        ...
        if [ -n "${W_OPT_UNATTENDED}" ]; then
            ...
            "$WINE" dotnetfx.exe ${W_OPT_UNATTENDED:+/q /c:"install.exe /q"}
            status=$?
        fi
...
load_dotnet471()
{
    ...
    WINEDLLOVERRIDES=fusion=b "$WINE" "$file1" ${W_OPT_UNATTENDED:+$unattended_args}
    status=$?
...
w_metadata opensymbol fonts \
    ...
    year="2017" \
    media="download" \
    file1="fonts-opensymbol_102.10+LibO6.1.5-3+deb10u2_all.deb" \
...
load_opensymbol()
{
    ...
    w_download "https://deb.debian.org/debian-security/pool/updates/main/libr/libreoffice/fonts-opensymbol_102.10+LibO6.1.5-3+deb10u2_all.deb" ba29052c85392e373478d5cf59199383a622257392888cef47b62b2df8868670
